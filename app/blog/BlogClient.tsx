...
export default function BlogClient({ initialPosts }: { initialPosts: Post[] }) {
  ...
  const cats = ['All', ...Array.from(new Set(initialPosts.map(p => p.category).filter(Boolean))).sort()]
    ...
    <div>
      ...
      {filtered.length === 0 && (
        <div style={{ padding:'4rem clamp(1.25rem,5vw,3.5rem)', textAlign:'center', color:'var(--color-text-tertiary)' }}>
          {initialPosts.length === 0 ? 'No posts published yet.' : 'No posts in this category.'}
        </div>
      )}
    </div>
  )
