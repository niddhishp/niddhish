      ...
      <p key={i} style={{
        ...
        {parts.map((part, j) => {
          if (part.startsWith('**') && part.endsWith('**')) {
            return <strong key={j} style={{ color: 'var(--color-text-primary)', fontWeight: 500 }}>{part.replace(/\*\*/g, '')}</strong>
          }
          return part
        })}
      </p>
      ...
      <div style={{ padding: '0 clamp(1.25rem,5vw,3.5rem)', marginBottom: '4rem', maxWidth: 780 }}>
        <Link href="/blog" style={{
          ...
          <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
            <path d="M9 5H1M5 9L1 5l4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        ...
        <h1 style={{
          ...
          fontWeight: 400, lineHeight: 1.15, letterSpacing: '-0.025em',
          color: 'var(--color-text-primary)',
          marginBottom: '1.5rem',
        }}>
          {post.title}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '2rem' }}>
            {otherPosts.map(p => (
              <Link key={p.slug} href={`/blog/${p.slug}`} style={{
                ...
                <span style={{ fontSize: 10, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--color-accent)', display: 'block', marginBottom: '0.5rem' }}>
                  {p.category}
                </span>
                <h3 style={{ fontFamily: 'var(--font-playfair,serif)', fontSize: 16, fontWeight: 400, color: 'var(--color-text-primary)', lineHeight: 1.35 }}>
                  {p.title}
                </h3>
              </Link>
            ))}
